...
- [x] `arastirmaci-profili-olusturma` (benefits/basicSteps/tips/comparison/contact/platforms + quickNav) (2026-08-19)
  - **Karar: quickNav kaldırıldı** — sayfada zaten HybridTOC vardı; quickNav global QuickAccess (floating nav) bu sayfanın tek kullanıcısıydı, kısayol gezinmesi TOC ile karşılanıyor. `quickNav` veriden silindi → `app.initQuickAccess` artık bu sayfada çalışmıyor (QuickAccess bileşeni + `quick-access.css` global olarak korunuyor, ileride kullanılabilir)
  - 23 content section'a dönüşüm: 4 genel blok (benefits/basic-steps/tips/comparison) + 18 platform bölümü (orcid×2, wos×6, scopus×6, googleScholar×4) + iletisim
  - Eşleme: benefits→**yeni `numbered-list` bileşeni** (rozete sayı + başlık + açıklama); basicSteps→heading + content(intro) + 4 `collapsible-section` (ordered-list içinde); tips→`icon-list` (ikon+başlık+açıklama); comparison→mevcut `table` bileşeni; platformlar→heading (emoji title'a eklendi, subtitle varsa `double-plain`) + content + `info-box` + `alert`(warning) + `numbered-list` + `step-cards`; iletisim→heading + content + `staff-list` (email/telefon numaraları görünüyor)
  - **Ortak renderer eklemeleri:** yeni `numbered-list` bileşeni (renderer case + şema + `.component-numbered-list` CSS, eski `.benefit-*` görünümünü taşır); `step-cards` adım içi `blocks` dizisi desteği (expandable adım içine info-box/alert render, 4 adımda nested blok var)
  - Özel sayfa JS 514 satırdan 60 satıra (renderBenefits/BasicSteps/Tips/ComparisonTable/PlatformSections/Contact + step toggles + IntersectionObserver animasyonu silindi → `ComponentRenderer.buildSectionCards`); 898 satır bespoke sayfa CSS'i silindi (`.researcher-profile-page`, `.info-card`, `.platform-section`, `.step-*`, `.benefit-*`, `.tip-item`, `.comparison-table`, `.basic-step-*`, `.contact-*`, `.nav-group` örtüşmeleri — ortak bileşen class'ları karşılıyor), HTML'den css linki kaldırıldı
  - ✅ Doğrulama: node --check temiz, validation 0 yeni hata, site çapı render scan 28/28 temiz (duplicate/undefined id yok), smoke test (styled: numbered-list ×2, 30 step-card, 4'i nested blocks, staff-list, table, 4 collapsible, 18 emoji heading, 23 TOC anchor = 23 section id eksiksiz)
- [x] `home.sections` dinamik blok kararı + test (2026-08-19)
  - **Karar: home content-bileşen sistemine ÇEVRİLMEZ, config-drive dinamik blok mimarisi korunur.** Gerekçe: (1) home bölümleri statik içerik değil — collection tab switch, haber slider (otoplay/progress), arrivals carousel, otomatik modal, kopyala-yapıştır gibi etkileşim ortak renderer'ın üreteceği HTML ile karşılanamaz; (2) `home.json sections.*` zaten "dinamik blok ayar" katmanı (enabled/dataSource/maxItems/showTitle/viewAllButton/dataKey) ve içerik ayrı JSON'larda (`data/content/collections|services|arrivals.json`, `data/pages/guncel-haberler|duyurular.json`) — bu yapı bileşen globalizasyonuna paralel bir konfig sistemi olarak resmileştirildi; (3) ana sayfa şeması (`manager/schemas/home.json`) bu ayarları zaten tam doğruluyor
  - **Test:** yeni `manager/check_home_blocks.py` — her enabled bölümün dataSource'u çözülür, dosya var/parse edilebilir, beklenen alan (top-level liste veya newsItems/announcementItems/services/arrivals) dolu olduğu doğrulanır; `python manager/check_home_blocks.py` → 5/5 TEMİZ (collections 5, services 8, news 8 (maxItems 4), announcements 10, arrivals 8 (maxItems 5))
- [x] **Doğrulama altyapısı düzeltmesi:** `manager/validation.py` modülünün kendi main'i yoktu (eski `python manager/validation.py` iddiaları boştu) — gerçek tam site doğrulaması şema haritasıyla yapıldı: 42 dosya, yalnızca 5 mevcut hata (organizasyon-semasi.pdf ×1 + quickactions rehber-videolar.html ×4). Ortaya çıkan **2 yeni hata düzeltildi** (mendeley `contact-buttons.text` lang nesnesiydi, şema `text` istiyordu → `_components.json`'da `text` alanı `lang` tipine çevrildi, renderer zaten ikisini destekliyor); tüm dönüşümler gerçek doğrulamada 0 yeni hata üretiyor
