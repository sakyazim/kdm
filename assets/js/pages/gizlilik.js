/**
 * Anadolu Üniversitesi Kütüphane - Gizlilik Politikası Sayfası
...
export class GizlilikPage {
  ...
  async init() {
    Utils.log('GizlilikPage initializing...');
    ...
    Utils.log('GizlilikPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/gizlilik.json');
      if (response.ok) {
        this.pageData = await response.json();
        Utils.log('Gizlilik data loaded successfully');
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        ...
        description: 'Kütüphanemiz kullanıcı gizliliğini koruma taahhüdünü en üst düzeyde tutar.',
        icon: 'bi bi-shield-lock'
      },
      content: [],
      helpSection: {
        title: 'Gizlilik ile ilgili sorularınız mı var?',
