/**
 * Anadolu Üniversitesi Kütüphane - Üyelik & Ödünç İşlemleri Sayfası
...
export class UyelikOduncIslemleriPage {
  constructor(app) {
    this.app = app;
    this.data = app.data;
  ...
  async init() {
    Utils.log('UyelikOduncIslemleriPage initializing...');
    ...
    Utils.log('UyelikOduncIslemleriPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/uyelik-odunc-islemleri.json');
      if (response.ok) {
        this.pageData = await response.json();

        // Veriyi global olarak sakla (eski kod uyumluluğu için)
        window.oduncData = this.pageData;

        Utils.log('Üyelik & Ödünç İşlemleri data loaded successfully');
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        title: 'Üyelik & Ödünç İşlemleri',
        description: 'Kütüphane üyelik ve ödünç alma işlemleri hakkında bilgi edinin.',
  ...
  async setupContent() {
    const container = document.getElementById('odunc-content');

    if (!container) {
      console.warn('Odunc content container not found');
      return;
    }
...
export default UyelikOduncIslemleriPage;
