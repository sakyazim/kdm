/**
 * Anadolu Üniversitesi Kütüphane - Kullanım Koşulları Sayfası
...
export class KosullarPage {
  ...
  async init() {
    Utils.log('KosullarPage initializing...');
    ...
    Utils.log('KosullarPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/kosullar.json');
      if (response.ok) {
        this.pageData = await response.json();
        Utils.log('Kosullar data loaded successfully');
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        ...
        description: 'Kütüphane kaynaklarını ve hizmetlerini kullanırken uyulması gereken kurallar ve koşullar.',
        icon: 'bi bi-file-earmark-check'
      },
      content: [],
      helpSection: {
        title: 'Kullanım koşulları hakkında sorularınız mı var?',
