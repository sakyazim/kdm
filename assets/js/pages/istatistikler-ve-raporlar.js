/**
 * Anadolu Üniversitesi Kütüphane - İstatistikler ve Raporlar Sayfası
...
export class IstatistiklerVeRaporlarPage {
  constructor(app) {
    this.app = app;
    this.pageData = null;
  ...
  async init() {
    Utils.log('IstatistiklerVeRaporlarPage initializing...');
    ...
    Utils.log('IstatistiklerVeRaporlarPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/istatistikler-ve-raporlar.json');
      if (response.ok) {
        this.pageData = await response.json();
        Utils.log('İstatistikler ve Raporlar data loaded successfully');
      } else {
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        ...
        description: 'Kütüphane istatistikleri ve raporları.',
  ...
  async setupContent() {
    ...
    this.setupCircularProgressAnimation();
  }
  ...
  /**
   * Circular Progress animasyonlarını ayarla
   * Faaliyet raporundan uyarlanan animasyon
   */
  setupCircularProgressAnimation() {
    // Intersection Observer ile görünüme girdiğinde animasyonu başlat
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !entry.target.classList.contains('animated')) {
          const card = entry.target;
          const circle = card.querySelector('.progress-bar-circular');
    ...
    // Tüm circular progress card'ları gözlemle
    const cards = document.querySelectorAll('.circular-progress-card');
    cards.forEach(card => observer.observe(card));
  }
}

export default IstatistiklerVeRaporlarPage;
