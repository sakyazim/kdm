...
export class DuyurularPage {
  ...
  async init() {
    Utils.log('DuyurularPage initializing...');
    ...
    Utils.log('DuyurularPage initialized');
  }
  ...
  async loadPageData() {
    try {
      ...
      if (response.ok) {
        ...
        Utils.log('Duyurular page data loaded successfully');
      } else {
        console.warn('Failed to load Duyurular page data');
        this.loadFallbackPageData();
      }
    } catch (error) {
      console.error('Error loading Duyurular page data:', error);
      this.loadFallbackPageData();
    }
  }

  async loadAnnouncementData() {
    // Duyuru verileri pageData içinde announcementItems olarak geliyor
    if (this.pageData && this.pageData.announcementItems) {
      this.announcementData = this.pageData.announcementItems;
      this.filteredAnnouncements = [...this.announcementData];
      Utils.log('Announcement data loaded successfully:', this.announcementData.length, 'items');
    } else {
      console.warn('Failed to load announcement data from pageData');
      this.announcementData = [];
      this.filteredAnnouncements = [];
    }
  }
  ...
  loadFallbackPageData() {
    this.pageData = {
      hero: {
        title: 'Duyurular',
        description: 'Kütüphane duyuruları, önemli bildirimler ve resmi açıklamalar',
        icon: 'bi bi-megaphone-fill'
      },
      content: [],
      helpSection: {
        title: 'Yardıma mı ihtiyacınız var?',
        description: 'Duyurular hakkında daha fazla bilgi için iletişime geçin.',
        buttons: []
      }
    };
  }
  ...
  renderContent(contentSections) {
    ...
    contentSections.forEach(section => {
      ...
      if (sectionContainer && section.components) {
        ...
        const isAnnouncementsSection = section.id === 'announcements-section';
        ...
        // Announcements section özel durum - heading + news-grid container
        if (isAnnouncementsSection) {
          // Heading'i render et
          if (hasHeading) {
          ...
          // News grid ve no-results container'larını ekle
          bodyContainer.innerHTML = `
            <div id="news-grid" class="news-grid list-view"></div>
            <div class="no-results" id="no-results" style="display: none;">
              <i class="fas fa-search"></i>
              <div class="no-results-content">
                <h4>${Utils.getLocalizedText(this.pageData.content[0].components[0].data.noResults.title)}</h4>
                <p>${Utils.getLocalizedText(this.pageData.content[0].components[0].data.noResults.description)}</p>
              </div>
            </div>
          `;
          return;
  ...
  /**
   * Duyuruları grid/list'de render et
   */
  renderAnnouncements() {
    ...
    const sortedAnnouncements = [...this.filteredAnnouncements].sort((a, b) => {
    ...
    sortedAnnouncements.forEach(announcement => {
      ...
      const hasRealLink = announcement.url && announcement.url !== '#';
    ...
    Utils.log(`${sortedAnnouncements.length} announcements rendered in ${this.currentView} view`);
  }
  ...
  openAnnouncementModal(announcement) {
    ...
    const modalContent = `
      <div class="news-modal">
        <img src="${announcement.image}" alt="${title}" class="news-modal-image" data-lightbox-trigger>
    ...
    if (lightboxTrigger) {
      lightboxTrigger.addEventListener('click', () => {
        this.openLightbox(announcement.image, announcement.title);
      });
    }
  ...
  /**
   * Duyuruları filtrele (kategori + arama)
   */
  filterAnnouncements() {
    this.filteredAnnouncements = this.announcementData.filter(announcement => {
      // Kategori filtresi
      const categoryMatch = this.currentCategory === 'all' || announcement.category === this.currentCategory;
    ...
    // Heading'i güncelle
    const headingTitle = document.querySelector('#announcement-heading .heading-title');
    if (headingTitle) {
      const categoryText = this.currentCategory === 'all' ? 'Tüm Duyurular' : this.currentCategory;
      const countText = `(${this.filteredAnnouncements.length})`;
  ...
  /**
   * URL parametrelerini kontrol et ve varsa ilgili duyuruyu otomatik aç
   * Kullanım: duyurular.html?id=5
   */
  checkUrlParams() {
    ...
    if (announcementId) {
      ...
      const announcement = this.announcementData.find(a => a.id === id);
      ...
      if (announcement) {
        ...
        Utils.log(`Auto-opening announcement modal for ID: ${id}`);
      } else {
