/**
 * Anadolu Üniversitesi Kütüphane - Bilgisayar Laboratuvarı Sayfası
...
export class BilgisayarLaboratuvariPage {
  constructor(app) {
    this.app = app;
    this.data = app.data;
  ...
  async init() {
    Utils.log('BilgisayarLaboratuvariPage initializing...');
    ...
    Utils.log('BilgisayarLaboratuvariPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/bilgisayar-laboratuvari.json');
      if (response.ok) {
        this.pageData = await response.json();

        // Veriyi global olarak sakla
        window.bilgisayarLaboratuvariData = this.pageData;

        Utils.log('Bilgisayar Laboratuvari data loaded successfully');
      } else {
...
export default BilgisayarLaboratuvariPage;