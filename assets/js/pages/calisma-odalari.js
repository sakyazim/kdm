/**
 * Anadolu Üniversitesi Kütüphane - Çalışma Odaları Sayfası
...
export class CalismaOdalariPage {
  constructor(app) {
    this.app = app;
    this.data = app.data;
  ...
  async init() {
    Utils.log('CalismaOdalariPage initializing...');
    ...
    Utils.log('CalismaOdalariPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/calisma-odalari.json');
      if (response.ok) {
        this.pageData = await response.json();

        // Veriyi global olarak sakla
        window.calismaOdalariData = this.pageData;

        Utils.log('Calisma Odalari data loaded successfully');
      } else {
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        ...
        description: 'Akademik personel ve lisansüstü öğrenciler için özel çalışma alanları',
        icon: 'fas fa-door-open'
      },
      content: [],
      helpSection: {
        title: 'Daha fazla yardıma mı ihtiyacınız var?',
...
export default CalismaOdalariPage;
