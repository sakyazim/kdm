/**
 * Anadolu Üniversitesi Kütüphane - İletişim Sayfası
...
export class IletisimPage {
  ...
  async init() {
    Utils.log('IletisimPage initializing...');
    ...
    Utils.log('IletisimPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/iletisim.json');
      if (response.ok) {
        this.pageData = await response.json();

        // Veriyi global olarak sakla
        window.iletisimData = this.pageData;

        Utils.log('Iletisim data loaded successfully');
      } else {
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        title: 'İletişim',
        description: 'Anadolu Üniversitesi Kütüphane ve Dokümantasyon Dairesi ile iletişime geçin.',
