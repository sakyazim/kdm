/**
 * Anadolu Üniversitesi Kütüphane - Formlar ve Dökümanlar Sayfası
...
export class FormlarPage {
  ...
  async init() {
    Utils.log('FormlarPage initializing...');
    ...
    Utils.log('FormlarPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('data/pages/formlar.json');
      if (response.ok) {
        this.pageData = await response.json();
        Utils.log('Formlar data loaded successfully');
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        title: 'Formlar ve Dökümanlar',
        description: 'Kütüphane hizmetleri için gerekli formlar, başvuru belgeleri ve kılavuzlar.',
        icon: 'bi bi-file-earmark-text'
      },
      content: [],
      helpSection: {
        title: 'Form konusunda yardıma mı ihtiyacınız var?',
