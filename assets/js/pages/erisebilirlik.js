...
export class ErisebilirlikPage {
  constructor(app) {
    this.app = app;
    this.pageData = null;
  ...
  async init() {
    Utils.log('ErisebilirlikPage initializing...');
    ...
    Utils.log('ErisebilirlikPage initialized');
  }

  async loadPageData() {
    try {
      const response = await fetch('assets/data/erisebilirlik.json');
      if (response.ok) {
        this.pageData = await response.json();
        Utils.log('Erisebilirlik data loaded successfully');
  ...
  loadFallbackData() {
    this.pageData = {
      hero: {
        ...
        description: 'Anadolu Üniversitesi Kütüphanesi, tüm kullanıcılar için erişilebilir bir deneyim sunmaya kararlıdır.',
...
export default ErisebilirlikPage;
